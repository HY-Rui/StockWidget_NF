...
class App(QApplication):
    def __init__(self, argv):
        ...
        app_icon = self._resolve_icon(icon_choice, self.style())
    ...
    @staticmethod
    def _resolve_icon(choice, style=None):
        """Resolve an icon choice to a QIcon.
        `choice` can be None/'default', 'std:NAME' or a file path.
        `style` is a QStyle used to fetch standard fallback icons.
        """
        # choice can be None, 'default', 'std:NAME' or a file path
    ...
    def set_app_icon(self, choice):
        ...
        icon = self._resolve_icon(choice, self.style())
        try:
