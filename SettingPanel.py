...
class SettingsDialog(QDialog):
    def __init__(self, win: FloatLabel, parent: QWidget, app=None):
        ...
        # 0.市场切换（股票 / 国内期货）
        g_market = QGroupBox("市场")
        g_market.setContentsMargins(3,12,3,6)
        lay_market = QHBoxLayout(g_market)
        lay_market.setSpacing(8)
        self.cmb_market = QComboBox()
        self.cmb_market.setFixedWidth(136)
        self.cmb_market.addItem("股票", userData="stock")
        self.cmb_market.addItem("国内期货", userData="futures")
        idx_mkt = self.cmb_market.findData(getattr(self.win, 'market', 'stock'))
        self.cmb_market.setCurrentIndex(idx_mkt if idx_mkt >= 0 else 0)
        lay_market.addWidget(QLabel("品种："))
        lay_market.addWidget(self.cmb_market)
        lay_market.addStretch(1)
        self.lbl_market_hint = QLabel("")
        self.lbl_market_hint.setStyleSheet("color:#888;")
        lay_market.addWidget(self.lbl_market_hint)
        code_settings.addWidget(g_market)
        ...
        for s in [0.2, 0.3, 0.5, 1, 2, 3, 5, 10, 15, 30, 60]:
            label = f"{s} 秒" if s != int(s) else f"{int(s)} 秒"
            self.cmb_interval.addItem(label, userData=float(s))
        # 旧配置可能是 int，统一按数值匹配
        cur = float(self.win.refresh_seconds)
        idx = -1
        for i in range(self.cmb_interval.count()):
            if abs(self.cmb_interval.itemData(i) - cur) < 1e-9:
                idx = i; break
        self.cmb_interval.setCurrentIndex(idx if idx >= 0 else 4)  # 默认 1 秒
        ...
        # 期货名称缩写（品种缩写+月份，如 螺纹钢2610→RB2610）
        self.cb_fut_abbrev = QCheckBox("缩写(期货)")
        self.cb_fut_abbrev.setChecked(bool(getattr(self.win, 'fut_abbrev', False)))
        self.cb_fut_abbrev.setToolTip("期货品种名用缩写+月份：螺纹钢2610→RB2610")
        self.cb_fut_abbrev.setEnabled(getattr(self.win, 'market', 'stock') == 'futures' and self.win.header_is_visible("名称"))
        gl_flag_name.addWidget(self.cb_fut_abbrev, 2, 0, 1, 2)
        ...
        self.cb_fut_abbrev.stateChanged.connect(self._on_fut_abbrev_toggled)

    def _current_market(self):
        return self.cmb_market.currentData() or "stock"

    def _update_market_hint(self):
        """根据当前市场更新代码输入提示。"""
        if self._current_market() == "futures":
            self.lbl_market_hint.setText("如 nf_RB2610")
            self.lbl_market_hint.setToolTip("国内期货统一用 nf_ 前缀\n示例：nf_RB2610(螺纹) nf_M2609(豆粕)\nnf_IF2606(股指) nf_TA2609(PTA) nf_SI2612(工业硅)")
        else:
            self.lbl_market_hint.setText("如 600000")
            self.lbl_market_hint.setToolTip("股票代码：sh/sz/bj 前缀或6位数字\n示例：600000(浦发), 000001(平安), sh000001(上证)")

    def _rebuild_codes_list(self):
        """根据浮窗当前市场的 codes/checked_codes 重建自选列表 UI。
        初始化与切换市场时共用。"""
        self.list_codes.blockSignals(True)
        self.list_codes.clear()
        checked = getattr(self.win, 'checked_codes', [])
        for c in self.win.codes:
            ...
            it.setCheckState(Qt.Checked if c in checked else Qt.Unchecked)
    ...
    def _on_market_changed(self, idx: int):
        mode = self.cmb_market.itemData(idx) or "stock"
        self._update_market_hint()
        # 切换市场：浮窗切换到目标市场的独立自选列表
        try:
            self.win.set_market(mode)
        except Exception:
            pass
        # 用目标市场的自选列表重建 UI（股票/期货各自独立，互不干扰）
        self._rebuild_codes_list()
        # 缩写(期货) 仅期货模式可用
        self.cb_fut_abbrev.setEnabled(mode == 'futures' and self.win.header_is_visible("名称"))
    ...
    # 国内期货代码格式：统一 nf_ 前缀 + 合约(字母+数字)
    # 新浪期货接口国内各交易所都用 nf_（hf_ 是外盘期货）
    # 例：nf_RB2610(螺纹), nf_M2609(豆粕), nf_IF2606(中金股指), nf_TA2609(郑商PTA), nf_SI2612(工业硅)
    _re_futures = re.compile(r'^nf_[a-zA-Z]{1,3}\d{3,4}$', re.IGNORECASE)

    def _normalize_code_or_none(self, s: str):
        # 期货模式：要求输入完整代码，仅做格式校验
        # 注意：新浪期货接口对合约字母大小写敏感，品种字母必须大写（nf_RB2610 有数据，nf_rb2610 为空）
        if self._current_market() == "futures":
            s = (s or "").strip()
            if not s:
                return None
            m = self._re_futures.match(s)
            if m:
                # 统一为：前缀 nf_ 小写 + 合约品种字母大写
                # 例：nf_rb2610/NF_RB2610/Nf_Rb2610 -> nf_RB2610
                core = s[3:].upper()      # 去掉前3字符(任意大小写的nf_)后大写合约部分
                return "nf_" + core
            return None
    ...
    def _collect_codes_from_list(self):
        codes = []
        seen = set()
        # 正在编辑的项跳过校验：编辑过程中的中间态文本（如 "nf_A"）是正常的，
        # 不能当成无效代码回弹/删除，否则会打断用户输入
        editing_item = None
        cur = self.list_codes.currentItem()
        if self.list_codes.state() == QAbstractItemView.EditingState and cur is not None:
            editing_item = cur
    ...
    def _add_code(self):
        # 添加时按当前市场给默认示例代码
        if self._current_market() == "futures":
            default_code = "nf_RB2610"
        else:
            default_code = "sh000001"
        it = QListWidgetItem(default_code)
        it.setFlags(it.flags() | Qt.ItemIsUserCheckable | Qt.ItemIsEditable | Qt.ItemIsSelectable | Qt.ItemIsEnabled)
        it.setCheckState(Qt.Unchecked)
        it.setData(Qt.UserRole, default_code)
        self.list_codes.addItem(it)
    ...
    def _on_cb_changed(self, header: str, state: bool):
        ...
        if header == "代码":
            ...
            self.cb_fut_abbrev.setEnabled(state and getattr(self.win, 'market', 'stock') == 'futures')
    
    def _on_short_code_toggled(self, checked: bool):
        self.win.set_code_type(checked)

    def _on_fut_abbrev_toggled(self, checked: bool):
        self.win.set_fut_abbrev(bool(checked))
